#
# vpn-reassign-hosts.py
#
# Abre el fichero reassign-hosts-users.csv y para cada registro
# genera los ficheros username.yml e address.yml.
#
# El fichero hosts-users.csv tiene el siguiente formato:
#
# address,mask,privatekey,userpath,username,usercomment,password,olduser
# 10.0.0.160,/20,aabbcc,users/alumnes,usuario1,usuario1 usuario1,secreto1,olduser1
# 10.0.0.161,/20,bbccaa,users/alumnes,usuario2,usuario2 usuario2,secreto2,olduser2
# 10.0.0.162,/20,ccaabb,users/alumnes,usuario3,usuario3 usuario3,secreto3,olduser3
#
#
#
...
with open('reassign-hosts-users.csv', newline='') as csvfile:
...
"""
        ...
        f.write(template.render(privatekey=row["privatekey"], address=row["address"], mask=row["mask"], userfile=os.path.join(row["userpath"],row["username"] + ".yml"), olduser=row["olduser"]))
        f.close()
